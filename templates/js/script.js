const { jsPDF } = window.jspdf;
      ...
      async function populateManageReportsPage() {
        ...
        const supplierSelect = document.getElementById('manage-reports-supplier-filter');

        // --- PERUBAHAN DI SINI: Mengisi dropdown supplier saat pertama kali dijalankan ---
        if (supplierSelect.options.length <= 1) { // Cek agar tidak diisi berulang kali
            if (AppState.ownerData && AppState.ownerData.supplier_data) {
                AppState.ownerData.supplier_data.forEach(s => {
      ...
      // Pastikan Anda juga memiliki fungsi updateProductSelection ini
      // (Sepertinya fungsi ini belum ada, ini diperlukan agar checkbox produk muncul)
      // GANTI FUNGSI LAMA DENGAN VERSI BARU INI
      // PASTIKAN FUNGSI INI SAMA SEPERTI DI BAWAH
      ...
      // (di bawah fungsi openAturProdukModal)

      // === TAMBAHKAN FUNGSI BARU INI ===
      // GANTI FUNGSI LAMA DI FILE ANDA DENGAN VERSI BARU INI
      // GANTI FUNGSI LAMA DENGAN VERSI BARU INI
      async function handleAddNewProduct(e) {
        ...
        try {
            ...
            if (result.success) {
                // Simpan produk baru ke state lokal (AppState)
                AppState.masterData.products.push(result.product);
                document.getElementById("new-product-name-input").value = '';

                // ================================================================
                // === INILAH LOGIKA BARU YANG MEMPERBAIKI MASALAH KONFLIK ===
                // ================================================================
                // Panggil ulang fungsi updateProductSelection dengan event palsu
                // untuk menggambar ulang seluruh daftar produk DENGAN data terbaru.
                // Ini memastikan fungsi pencarian juga ikut ter-update.
                updateProductSelection({ target: selectedSupplierRadio });
                
                // Setelah digambar ulang, cari checkbox produk baru dan centang.
                setTimeout(() => {
      ...
      function generateReportTables() {
        ...
        if (selectedCheckboxes.length === 0) {
            return showToast("Pilih setidaknya satu produk.", false);
        }
        ...
        selectedCheckboxes.forEach(cb => {
            ...
            if(product) productsToDisplay.push({ ...product, stokAwal: stokAwal });
        });

        if (hasInvalidStok) {
            return showToast("Stok awal untuk produk yang dipilih harus diisi dan lebih dari 0.", false);
        }
        
        // 1. HEADER TABEL DIPERBARUI: Kolom "Biaya" ditambahkan
        let tableHtml = `
                ...
                <table class="table table-bordered table-hover align-middle">
                    <thead class="table-light">
                        <tr>
                            ...
                            <th class="text-center" style="width: 8%">Terjual</th>
                            <th class="text-end" style="width: 15%">Pendapatan</th>
                            <th class="text-end" style="width: 15%">Biaya</th>
                            <th class="text-end" style="width: 15%">Keuntungan</th>
                            <th class="text-center" style="width: 12%">Notifikasi</th>
                        </tr>
                    </thead>
                    <tbody>
        `;
        ...
        productsToDisplay.forEach(productData => {
            ...
            rowHtml = rowHtml.replace(
                'class="form-control form-control-sm text-center input-stok stok-awal" placeholder="0"',
                `class="form-control-plaintext form-control-sm text-center input-stok stok-awal" value="${productData.stokAwal}" readonly`
            );
            tableHtml += rowHtml;
        });
        ...
        document.getElementById("product-search-container").style.display = 'block'; // Tampilkan input pencarian
        attachAllEventListeners(); 
        updateGrandTotals(); 
        
        modals.aturProduk.hide();
        showToast("Produk berhasil diterapkan ke tabel.");
      }
      ...
      function createProductRow(product, supplier) {
          ...
          // 2. BARIS TABEL DIPERBARUI: Kolom "Biaya" ditambahkan
          return `
              <tr class="product-row" data-product-id="${product.id}" data-harga-jual="${product.harga_jual}" data-harga-beli="${product.harga_beli}">
                  <td class="product-supplier-info">
                    <small class="fw-bold d-block">${supplier.name}</small>
                    <strong>${product.name}</strong>
                  </td>
                  <td>
                    <input type="number" class="form-control form-control-sm text-center input-stok stok-awal" placeholder="0">
                  </td>
                  ...
                  <td class="text-end fw-bold biaya-rp">${formatCurrency(0)}</td>
                  <td class="text-end fw-bold keuntungan-rp">${formatCurrency(0)}</td>
                  <td class="text-center">
                    <button class="btn btn-outline-warning btn-sm notify-btn">
                        <i class="bi bi-bell-fill"></i> Stok Habis
                    </button>
                  </td>
              </tr>`;
      }
        ...
        try {
            ...
            if (result.success) {
                ...
                button.innerHTML = '<i class="bi bi-check-lg"></i> Terkirim';
                 ...
                 button.disabled = false; // Aktifkan lagi jika gagal
      ...
      /*-0i// PERUBAHAN 5: Modifikasi fungsi createProductRow
      ...
      function updateRowAndTotals(row) {
          ...
          const biaya = terjual * hargaBeli; // 3. Logika perhitungan "Biaya"
          ...
          row.querySelector(".biaya-rp").textContent = formatCurrency(biaya);
          row.querySelector(".keuntungan-rp").textContent = formatCurrency(keuntungan);
      ...
      function updateGrandTotals() {
        ...
        document.querySelectorAll(".product-row").forEach((row) => {
            const awal = parseInt(row.querySelector(".stok-awal").value) || 0;
            const akhir = parseInt(row.querySelector(".stok-akhir").value) || 0;
            const hargaJual = parseFloat(row.dataset.hargaJual);
            const terjual = Math.max(0, awal - akhir);
            
            // Update kolom 'terjual' di barisnya
            row.querySelector(".terjual-pcs").textContent = terjual;

            totalPendapatan += terjual * hargaJual;
        });

        // Update footer rekapitulasi (logika ini tetap sama)
      ...
      function attachAllEventListeners() {
        ...
        // 4. Tambahkan event listener untuk input pencarian tabel utama
